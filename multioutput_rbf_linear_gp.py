...
def load_exact_model_bundle(save_path, gp_regression_dataset, device):
    ...
    if len(models_state_dict) != len(gp_regression_dataset):
        print(f"Warning: Loaded {len(models_state_dict)} models but expected {len(gp_regression_dataset)}")
...
def load_prioritized_bundles(partial_bundles, gp_regression_dataset, device):
    ...
    for bundle_dates, bundle_path, bundle_count in partial_bundles:
        ...
        if len(models) >= len(gp_regression_dataset):
            print(f"Reached target of {len(gp_regression_dataset)} models. Stopping bundle loading.")
            break
...
def train_all_models(gp_regression_dataset, device):
    ...
    # Create outer progress bar for overall tracking
    overall_pbar = tqdm(total=len(gp_regression_dataset), desc="Overall Progress", position=0)
    ...
    try:
        for idx in range(len(gp_regression_dataset)):
            ...
            if processed_count > 0:
                ...
                # Display progress summary
                print(f"Fitting GP Models: {processed_count}/{len(gp_regression_dataset)} completed")
                print(f"Average time per model: {avg_time_per_model:.2f} seconds")
                print(f"Estimated time remaining: {eta_str}")
            else:
    ...
    clear_output(wait=True)
    print(f"Model fitting complete! {len(models)}/{len(gp_regression_dataset)} models successfully fit")
    print(f"Total time elapsed: {int(hours):02d}:{int(minutes):02d}:{int(seconds):02d}")
    print(f"Average time per model: {total_time/processed_count:.2f} seconds")
    
    return models, likelihoods, losses

def load_or_train_models(date_range, gp_regression_dataset, device, reload_existing_models=True):
    ...
    if os.path.exists(save_path) and reload_existing_models:
        ...
        models, likelihoods, losses = load_exact_model_bundle(save_path, gp_regression_dataset, device)
        ...
        if partial_bundles:
            ...
            models, likelihoods, losses = load_prioritized_bundles(partial_bundles, gp_regression_dataset, device)
            models, likelihoods, losses = train_remaining_models(models, likelihoods, losses, gp_regression_dataset, device)
